...
class DeviceDisplay:
    def __init__(self, raw, draw_number=None, number_dimensions=None):
        self.raw = raw
    ...
    def print(self, text, scale=1):
        ...
        if scale > 1 and self._draw_number and _numeric(text):
            x, y = getattr(self, "_pos", (0, 0))
            try:
                self._draw_number(text, x, y, scale, self._fg, self._bg)
                return
            except Exception:
